...
const { CREATED, BAD_REQUEST, EXPECTATION_FAILED, OK, FORBIDDEN, UNAUTHORIZED } = StatusCodes;
...
describe('User Router:', () => {
  ...
  describe('/users/register', () => {
    ...
    it('can register users', async () => {
      ...
      }).set('Accept', 'application/json').expect(CREATED);
    ...
    it('send BAD_REQUEST if validation error', async () => {
      await request(server).post("/users/register").send({
        ...
        login: 'Test Login',
        password: 'testpassword'
      }).set('Accept', 'application/json').expect(BAD_REQUEST);
    })
    ...
    it('send EXPECTATIONS_FAILED if user already exists', async () => {
        ...
        await request(server).post("/users/register").send({
          ...
          login: 'Test Login',
          password: 'testpassword'
        }).set('Accept', 'application/json').expect(EXPECTATION_FAILED); 
  ...
  describe('/users/login', () => {
    ...
    it('receives OK Status Code', async () => {
      ...
      await request(server)
        .post('/users/login')
        .send(userMock)
        .set('Accept', 'application/json').expect(OK);
    ...
    it('send FORBIDDEN Status if authentication failed', async () => {
      ...
      .post('/users/login')
      .send(userMock)
      .set('Accept', 'application/json').expect(FORBIDDEN);
