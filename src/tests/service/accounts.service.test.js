...
describe('AccountService:', () => {
  ...
  const fakeAccountId = '6074688473a1e43514a3d60e';
  describe('get', () => {
    beforeEach(async () => {
  ...
  describe('getAll', () => {
    ...
    it('returns zero accounts correctly', async () => {
      const zerroAccounts = await AccountService.getAll(user._id);
      expect(zerroAccounts.length).toBe(0);
    });
  ...
  describe('create', () => {
    ...
    it('throws ENTITY_EXISTS error', async () => {
      ...
      await expect(AccountService.create(accountMockData, user._id))
        .rejects.toThrowError(ENTITY_EXISTS);
    });
